/* »м€: hiddata.c
 * јвтор: Christian Starkjohann
 * ѕеревод: microsin.ru
 * ƒата создани€: 2008-04-11
 * “абул€ци€: 4
 * Copyright: (c) 2008 by OBJECTIVE DEVELOPMENT Software GmbH
 * Ћицензи€: GNU GPL v2 (см. License.txt) или проприетарна€ (CommercialLicense.txt) 
 * –евизи€: $Id: hiddata.c 553 2008-04-17 19:00:20Z cs $
 */
...
{
GUID                                hidGuid;        /* GUID дл€ драйвера HID */
...
HIDD_ATTRIBUTES                     deviceAttributes;
    ...
    for(i=0;;i++){
        ...
        /* сначала делаем dummy-вызов, только чтобы определить требуемый объем данных */
        SetupDiGetDeviceInterfaceDetail(deviceInfoList, &deviceInfo, NULL, 0, &size, NULL);
        if(deviceDetails != NULL)
            free(deviceDetails);
        deviceDetails = malloc(size);
        deviceDetails->cbSize = sizeof(*deviceDetails);
        /* этот вызов уже "взаправду": */
        SetupDiGetDeviceInterfaceDetail(deviceInfoList, &deviceInfo, deviceDetails, size, &size, NULL);
        DEBUG_PRINT(("checking HID path \"%s\"\n", deviceDetails->DevicePath));
        /* попытка на открытие дл€ R/W -- нам не нужно заботитьс€ об устройствах, которые не могут быть доступны */
        ...
        if(handle == INVALID_HANDLE_VALUE){
            ...
            /* errorCode = USBOPEN_ERR_ACCESS; открытие дл€ мыши будет всегда с ошибкой -- игнорируем */
            continue;
        }
        ...
        break;  /* мы нашли устройство, которое искали! */
    }
...
int     rval, i;
    ...
    if((rval = usb_get_string_simple(dev, index, buf, buflen)) >= 0) /* используйте версию libusb, если это работает */
        return rval;
    ...
    rval /= 2;
    /* конверси€ с потер€ми в кодировку ISO Latin1: */
    for(i=1;i<rval;i++){
        if(i > buflen)              /* переполнение буфера назначени€ */
            break;
        buf[i-1] = buffer[2 * i];
        if(buffer[2 * i + 1] != 0)  /* выход за пределы диапазона ISO Latin1 */
            buf[i-1] = '?';
    }
    buf[i-1] = 0;
    return i-1;
    ...
    for(bus=usb_get_busses(); bus; bus=bus->next){
        for(dev=bus->devices; dev; dev=dev->next){
            if(dev->descriptor.idVendor == vendor && dev->descriptor.idProduct == product){
                char    string[256];
                int     len;
                handle = usb_open(dev); /* нам нужно открыть устройство дл€ запроса строк */
                if(!handle){
                    errorCode = USBOPEN_ERR_ACCESS;
                    fprintf(stderr, "Warning: cannot open USB device: %s\n", usb_strerror());
                    continue;
                }
                if(vendorName == NULL && productName == NULL){  /* не беспокоимс€ об имени */
                    break;
                }
                /* теперь проверим, совпадают ли имена: */
...
int bytesSent;
    ...
    if(!usesReportIDs){
        buffer++;   /* пропуск dummy репорт ID */
        len--;
    }
...
int bytesReceived, maxLen = *len;
    ...
    if(!usesReportIDs){
        buffer++;   /* оставл€ем место под dummy репорт ID */
        maxLen--;
    }
    ...
    *len = bytesReceived;
    if(!usesReportIDs){
        buffer[-1] = reportNumber;  /* добавление dummy репорт ID */
        *len++;
    }
    return 0;
}
