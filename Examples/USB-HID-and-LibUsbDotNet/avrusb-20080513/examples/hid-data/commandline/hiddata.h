...
/*
Основное описание:
Этот модуль реализует слой абстракции для передачи данных через запросы HID feature. 
Реализация использует родные, встроенные в Windows функции (не требуется инсталляция
драйвера) и libusb на Unix. Вы должны линковать соответствующие библиотеки командами: 
"-lhid -lusb -lsetupapi" на Windows и `libusb-config --libs` на Unix.
*/
