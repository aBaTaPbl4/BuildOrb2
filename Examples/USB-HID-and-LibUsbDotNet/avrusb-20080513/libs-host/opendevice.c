...
/*
Основное описание:
Функции в этом модуле могут использоваться для нахождения и открытия устройства,
основываясь на libusb или libusb-win32.
*/
...
/* частный (private) интерфейс: сравнивается text и p, возвращает MATCH_SUCCESS, MATCH_FAILED, или MATCH_ABORT. */
...
int last, matched, reverse;
    ...
    for(; *p; text++, p++){
        ...
        switch(*p){
        case '\\':
            /* Литеральное соответствие со следующим символом. */
            p++;
            /* FALLTHROUGH */
        default:
            if(*text != *p)
        ...
        case '*':
            while(*++p == '*')
                /* последующие звездочки работают как одна. */
                continue;
            if(*p == 0)
                /* Конечная звездочка соответствует любому тексту. */
...
/* Публичный (public) интерфейс для совпадения в стиле шелла (shell): возвращает 0 если, 1 if matches */
static int shellStyleMatch(char *text, char *pattern)
{
    if(pattern == NULL) /* паттерн NULL - синоним "*" */
        return 1;
...
int     rval, i;
    ...
    rval /= 2;
    /* конверсия с потерями (lossy) в ISO Latin1: */
    for(i=1;i<rval;i++){
        if(i > buflen)              /* переполнение буфера назначения */
            break;
        buf[i-1] = buffer[2 * i];
        if(buffer[2 * i + 1] != 0)  /* выход за диапазон ISO Latin1 */
            buf[i-1] = '?';
    }
    buf[i-1] = 0;
    return i-1;
...
int                 errorCode = USBOPEN_ERR_NOTFOUND;
    ...
    if(printMatchingDevicesFp != NULL)  /* никогда возвращаем ошибку только для распечатки */
        errorCode = 0;
    return errorCode;
}
